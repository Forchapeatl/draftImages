...
/*  bindTextures() {
    ...
    const boundTexture = gl.getParameter(gl.TEXTURE_BINDING_2D);
          console.log(boundTexture);

    if (gl.getParameter(gl.TEXTURE_BINDING_2D) || gl.getParameter(gl.TEXTURE_BINDING_CUBE_MAP)||
    gl.getParameter(gl.TEXTURE_BINDING_3D) || gl.getParameter(gl.TEXTURE_BINDING_2D_ARRAY) ||
    gl.getParameter(gl.ARRAY_BUFFER_BINDING) || gl.getParameter(gl.ELEMENT_ARRAY_BUFFER_BINDING) ||
    gl.getParameter(gl.COPY_READ_BUFFER_BINDING) || gl.getParameter(gl.COPY_WRITE_BUFFER_BINDING) ||
    gl.getParameter(gl.TRANSFORM_FEEDBACK_BUFFER_BINDING) || gl.getParameter(gl.TRANSFORM_FEEDBACK_BUFFER) ||
    gl.getParameter(gl.UNIFORM_BUFFER_BINDING) || gl.getParameter(gl.FRAMEBUFFER_BINDING) ||
    gl.getParameter(gl.gl.READ_FRAMEBUFFER_BINDING) || gl.getParameter(gl.DRAW_FRAMEBUFFER_BINDING) ||
    gl.getParameter(gl.RENDERBUFFER_BINDING) || gl.getParameter(gl.VERTEX_ARRAY_BINDING)=== tex) {
    boundedTextures.add(tex);

      console.log(`Texture ${uniform.name} successfully bound.`);
    } else {
      console.log(`Failed to bind texture ${uniform.name}.`);
    }
      gl.uniform1i(uniform.location, uniform.samplerIndex);
    }
  } */
